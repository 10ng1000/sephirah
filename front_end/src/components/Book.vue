...
const isLink = ref(linkedBooks.value.includes(props.id))
...
function changeLinkAndSubmit() {
  if(chatSession.value == null){
    //如果当前没有chatSession，则把链接保存下来
    if (isLink.value) linkedBooks.value.push(props.id)
    else linkedBooks.value = linkedBooks.value.filter(id => id != props.id)
    return
  }
  if (isLink.value) {
    axios.post(`api/chat/sessions/${chatSession.value}/books`, { book_id: props.id }).then(
      () => {
        toast.success("链接成功")
        updateLinkedBooks(chatSession.value);
      }
    ).catch(
    ...
    axios.delete(`api/chat/sessions/${chatSession.value}/books/${props.id}`).then(
      () => {
...
<template>
  <article @click.self="viewBook()" @mouseenter="isHover = true" @mouseleave="isHover = false">
    ...
    <label class="toggle-button" :for="'check-' + props.id" ref="toggle" @mouseenter="showTooltip = true"
      @mouseleave="showTooltip = false"></label>
    ...
    <div class="button-group">
      ...
      <form method="post" action enctype="multipart/form-data" @submit.prevent="editBook()" v-if="Files != null">
